  ...
  return students
    .map((student) => {
      ...
      return revenueRaw.map((el) => ({
        ...
        prescribed_money: calculatorPrescribedMoney(
          el,
          student,
          batch,
          end_date
...
export async function PUT(req) {
  ...
  try {
    ...
    const sql_batch = await client.query(
      `SELECT * FROM batchs WHERE id = ${batch_id}`
    );
    const batch = sql_batch.rows[0].batch;
    const sql_school_year = await client.query(
    ...
    // Map revenue objects
    const objects = mapRevenueObjects(
      students,
      revenueNorms,
      batch_id,
      userId,
