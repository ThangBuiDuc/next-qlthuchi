...
function mergeElementArray(arr) {
  ...
  arr
    ...
    .forEach((item) => {
      let index = arrReturn.findIndex(
        (el) => el.student.student_code === item.student.student_code
      );
      if (index === -1) {
        arrReturn.push(item);
      } else {
      ...
      >
        {data.map((item, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="whitespace-nowrap">{`${item.student.first_name} ${item.student.last_name}`}</TableCell>
