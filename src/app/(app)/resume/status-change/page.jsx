...
const Page = async () => {
  const pathName = "/resume/status-change";
  ...
  const [apiListSearch, present, apiConfig, apiGetCatalogStudent] =
    await Promise.all([
      getListSearch(),
  ...
  if (
    ...
    apiGetCatalogStudent.status !== 200
  )
    throw new Error("Đã có lỗi xảy ra. Vui lòng thử lại!");
  ...
  return (
    <Content
      ...
      catalogStudent={apiGetCatalogStudent.data}
    />
  );
};
